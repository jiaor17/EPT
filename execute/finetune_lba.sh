...
SPLIT="${SPLIT:-ID30}" # default using ID30
echo "Using GPUs: $GPU"
...
########## start training ##########
cd $CODE_DIR
python train.py --gpus "${!GPU_ARR[@]}" --config ${CONFIG}/${SPLIT}/train.yaml pretrain_ckpt=${CKPT}


########## evaluation ##########
python -m evaluation.eval_lba --config ${CONFIG}/${SPLIT}/test.yaml --ckpt ./ckpts/LBA/${SPLIT} --gpu "${!GPU_ARR[@]}"