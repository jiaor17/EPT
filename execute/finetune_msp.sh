...
########## start training ##########
cd $CODE_DIR
python train.py --gpus "${!GPU_ARR[@]}" --config ${CONFIG}/train.yaml pretrain_ckpt=${CKPT}


########## evaluation ##########
python -m evaluation.eval_msp --config ${CONFIG}/test.yaml --ckpt ./ckpts/MSP --gpu "${!GPU_ARR[@]}"