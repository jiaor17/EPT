...
@R.register('PredictorNNDModel')
class PredictorNNDModel(nn.Module):

    def __init__(self, encoder: dict, graph_constructor: dict, noise_type: str,
                sigma_begin=0.01, sigma_end=10, n_noise_level=50, reduce_type="std_sum_block",
    ...
    def forward(self, Z, B, A, atom_positions, block_lengths, lengths, segment_ids, label, return_loss=True) -> ReturnValue:
        ...
        if self.training and self.noise_type != "none":
            # prepare data to concat
            ...
            H_input = torch.cat([H_0, H_0], dim=0)
            Z_input = torch.cat([Z, Z_perturbed], dim=0)
            block_id_input = torch.cat([block_id, block_id + block_id.max() + 1], dim=0)
            batch_id_input = torch.cat([batch_id, batch_id + batch_id.max() + 1], dim=0)
            edges_input = torch.cat([edges, edges + edges.max() + 1], dim=1)
            edge_attr_input = torch.cat([edge_attr, edge_attr], dim=0)
            unit_repr, block_repr, graph_repr, pred_Z = self.encoder(H_input, Z_input, block_id_input, batch_id_input, edges_input, edge_attr_input)   
            unit_repr, block_repr, graph_repr = unit_repr[:unit_repr.shape[0] // 2], block_repr[:block_repr.shape[0] // 2], graph_repr[:graph_repr.shape[0] // 2]
            if self.reduce_type == "std_sum_block":
                pred_energy = std_conserve_scatter_sum(self.energy_ffn(block_repr), batch_id, dim=0).squeeze(-1)
            elif self.reduce_type == "std_sum_unit":
                pred_energy = std_conserve_scatter_sum(self.energy_ffn(unit_repr), graph.batch_ids[graph.unit2block], dim=0).squeeze(-1)
            elif self.reduce_type == "sum_unit":
                pred_energy = scatter_sum(self.energy_ffn(unit_repr), graph.batch_ids[graph.unit2block], dim=0).squeeze(-1)           
            pred_noise = (pred_Z[H_0.shape[0]:] - Z_perturbed).view(-1, 1, 3)
