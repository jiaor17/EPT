...
def load_from_ckpt(model, ckpt):
    ...
    if isinstance(ori_model, dict):
        ...
        model.graph_constructor.load_state_dict(gen_state_dict(ori_model_dict, 'graph_constructor'))
        model.encoder.load_state_dict(gen_state_dict(ori_model_dict, 'encoder'))
    else:
        strict = not model.encoder.encoder.use_ieconv
        model.graph_constructor.load_state_dict(ori_model.graph_constructor.state_dict())
        model.encoder.load_state_dict(ori_model.encoder.state_dict(), strict=strict)
...
def main(args, overrides):
    ...
    ########## multi-pack wrapper #########
    if max_n_vertex_per_gpu > 0:
        train_sampler = MultipackDistributedBatchSampler(batch_max_length=max_n_vertex_per_gpu, lengths=train_set._lengths, seed=args.seed)
        valid_max_n_vertex_per_gpu = dataloader_cfg.get('valid_max_n_vertex_per_gpu', max_n_vertex_per_gpu)
        if valid_set is not None:
            valid_sampler = MultipackDistributedBatchSampler(batch_max_length=valid_max_n_vertex_per_gpu, lengths=valid_set._lengths, seed=args.seed)
        dataloader_cfg['batch_size'] = dataloader_cfg['valid_batch_size'] = 1
